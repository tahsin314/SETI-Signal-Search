...
from sklearn.metrics import roc_roc_auc_score
import torch, torchvision
from torch import optim
from torch.nn import functional as F
from torch.utils.data import Dataset,DataLoader
from SETIDataset import SETIDataset
...
valid_ds = SETIDataset(valid_df.image_name.values, valid_meta, valid_df.target.values, dim=sz, transforms=val_aug)
...
def evaluate():
   ...
   with torch.no_grad():
    for t in range(len(augs)):
      ...
      test_ds = SETIDataset(image_ids=valid_df.image_name.values, meta_features=valid_meta, dim=sz, transforms=augs[t])
...
def train_val(epoch, dataloader, optimizer, choice_weights= [0.8, 0.1, 0.1], rate=1):
  ...
  roc_auc = roc_roc_auc_score(lab, pred)
  ...
  submission = submission.sort_values(by=['difference'], ascending=False)
  submission.to_csv('val_report.csv', index=False)
  msg = f'Validation Loss: {running_loss/epoch_samples:.4f} Validation roc_auc: {roc_auc:.4f}'
  print(msg)
  return running_loss/epoch_samples, roc_auc
...
def evaluatev2(test_df, test_meta):
   ...
   with torch.no_grad():
     for t in range(len(augs)):
      print('TTA {}'.format(t+1))
      test_ds = SETIDataset(image_ids=test_df.image_name.values, meta_features=test_meta, dim=sz, transforms=augs[t])
...
if load_model:
  ...
  best_valid_roc_auc = tmp['best_roc_auc']
  print(best_valid_roc_auc)
  print('Model Loaded!')

# valid_loss, valid_roc_auc = train_val(-1, valid_loader, optimizer=optimizer, rate=1.00)
# evaluate()
